...
try {
    if ($action === 'save_note') {
        ...
        $pdo->prepare("UPDATE public.callback_notes
            ...
            ->execute([':notes'=>$notes,':fm'=>$fw_method,':fd'=>$fw_days,':fdate'=>$fw_date,
                       ':nby'=>current_username(),':id'=>$note_id,':uid'=>$uid,':admin'=>is_admin()?'TRUE':'FALSE']);
        $flash = "Notes saved.";
    }
    ...
    if ($action === 'set_status') {
        ...
        $pdo->prepare("UPDATE public.callback_notes SET status=:st, updated_at=NOW()
            WHERE id=:id AND (user_id=:uid OR :admin=TRUE)")
    ...
    if ($action === 'mark_done') {
        $note_id = (int)$_POST['note_id'];
        $pdo->prepare("UPDATE public.callback_notes SET status='done', done_at=NOW(), updated_at=NOW()
            WHERE id=:id AND (user_id=:uid OR :admin=TRUE)")
    ...
    if ($action === 'promote_interested') {
        $note_id = (int)$_POST['note_id'];
        $lead_id = (int)$_POST['lead_id'];
        $cid     = (int)$_POST['campaign_id'];
        // Insert into interested_notes
        ...
        // Update lead status
        $pdo->prepare("UPDATE public.leads SET status='interested' WHERE id=:id")
            ->execute([':id'=>$lead_id]);
        // Archive the callback entry
        $pdo->prepare("UPDATE public.callback_notes SET status='archived', updated_at=NOW() WHERE id=:id")
            ->execute([':id'=>$note_id]);
        $flash = "Lead promoted to Interested!";
    }

    if ($action === 'hard_delete' && is_admin()) {
        $note_id = (int)$_POST['note_id'];
        $pdo->prepare("DELETE FROM public.callback_notes WHERE id=:id")->execute([':id'=>$note_id]);
...
try {
    $af2 = is_admin() ? "" : "AND user_id = {$uid}";
    $rows = $pdo->query("SELECT status, COUNT(*) AS cnt FROM public.callback_notes WHERE 1=1 {$af2} GROUP BY status")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($rows as $r) {
        $s = $r['status'];
...
.sidebar{width:var(--sidebar-w);flex-shrink:0;background:#fff;border-right:1px solid var(--border);display:flex;flex-direction:column;position:fixed;top:0;left:0;bottom:0;z-index:50;box-shadow:2px 0 8px rgba(0,0,0,.04);}
...
.snav-item.active svg{stroke:var(--blue);}
.snav-badge{margin-left:auto;background:var(--green-lt);color:#065f46;border:1px solid #a7f3d0;border-radius:20px;font-size:10px;font-weight:700;padding:1px 6px;min-width:20px;text-align:center;}
.snav-badge-orange{background:var(--orange-lt);color:#9a3412;border-color:#fed7aa;}
...
.sidebar-username{font-size:12px;font-weight:600;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
.main-content{margin-left:var(--sidebar-w);flex:1;padding:28px;min-width:0;}
...
.page-title{font-size:22px;font-weight:800;margin-bottom:20px;color:#1e293b;}
...
.ftab.on{background:var(--orange);color:#fff;}
...
.b-active{background:var(--orange-lt);color:#9a3412;border:1px solid #fed7aa;}
...
.ib-promote{color:#7c3aed;border-color:#ddd6fe;background:#f5f3ff;}
.ib-promote:hover{background:#ede9fe;}
...
<div class="card">
    <div class="card-head">
        <span class="card-title">
            <?= match($filter) { 'archived'=>'Archived', 'done'=>'Done', default=>'Active Call Back Leads' } ?>
    ...
    <table style="min-width:600px;">
        <thead>
            <tr>
                ...
                <th style="width:<?= is_admin()?'145':'115' ?>px;">Actions</th>
            </tr>
        </thead>
        <tbody>
        <?php if (empty($leads)): ?>
        <tr><td colspan="<?= is_admin()?6:5 ?>"><div class="empty">No call back leads here yet.</div></td></tr>
        <?php endif; ?>
        <?php foreach ($leads as $l):
            ...
            $has_unread = !empty($unread_map[(int)$l['lead_id']]);
        ?>
        <tr style="<?= $is_paused?'opacity:.6;':'' ?><?= $has_unread?'background:#fff7ed;':'' ?>" onclick="openPanel(<?= (int)$l['id'] ?>)" title="Click to edit notes">
            <td style="font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= h((string)($l['business_name']??'')) ?>">
                <span class="unread-dot" style="display:inline-block;width:8px;height:8px;background:<?= $has_unread ? '#f97316' : 'transparent' ?>;border-radius:50%;margin-right:5px;vertical-align:middle;flex-shrink:0;"></span><?= $l['business_name'] ? h((string)$l['business_name']) : '<span style="color:var(--gray)">—</span>' ?>
            </td>
            <?php if (is_admin()): ?><td style="font-size:12px;color:var(--gray);"><?= h((string)($l['agent']??'—')) ?></td><?php endif; ?>
            ...
            <td>
                <div class="act-row">
                    ...
                    <?php if ($filter === 'active'): ?>
                        <!-- Promote to Interested -->
                        <form method="post" onsubmit="return confirm('Promote this lead to Interested?');" onclick="event.stopPropagation()">
                            <input type="hidden" name="action" value="promote_interested">
                            <input type="hidden" name="note_id" value="<?= (int)$l['id'] ?>">
                            <input type="hidden" name="lead_id" value="<?= (int)$l['lead_id'] ?>">
                            <input type="hidden" name="campaign_id" value="<?= (int)$l['campaign_id'] ?>">
                            <button type="submit" class="icon-btn ib-promote" title="Promote to Interested">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 11 12 6 7 11"/><polyline points="17 18 12 13 7 18"/></svg>
                            </button>
                        </form>
                        <!-- Mark Done -->
                        <form method="post" onsubmit="return confirm('Mark as done?');" onclick="event.stopPropagation()">
                        ...
                        <form method="post" onclick="event.stopPropagation()">
                            ...
                            <button type="submit" class="icon-btn ib-pause" title="<?= $is_paused?'Resume':'Pause' ?>">
                        ...
                        <!-- Archive -->
                        <form method="post" onsubmit="return confirm('Archive this lead?');" onclick="event.stopPropagation()">
                            <input type="hidden" name="action" value="set_status">
                            <input type="hidden" name="note_id" value="<?= (int)$l['id'] ?>">
                            <input type="hidden" name="new_status" value="archived">
                            <button type="submit" class="icon-btn ib-archive" title="Archive">
                    ...
                    <?php elseif ($filter === 'archived'): ?>
                        ...
                        <form method="post" onclick="event.stopPropagation()">
                            ...
                            <button type="submit" class="icon-btn ib-restore" title="Restore">
                        ...
                        <form method="post" onsubmit="return confirm('Permanently delete?');" onclick="event.stopPropagation()">
                            <input type="hidden" name="action" value="hard_delete">
                            <input type="hidden" name="note_id" value="<?= (int)$l['id'] ?>">
                            <button type="submit" class="icon-btn ib-del" title="Delete permanently">
                    ...
                    <?php elseif ($filter === 'done'): ?>
                        <form method="post" onclick="event.stopPropagation()">
                            ...
                            <input type="hidden" name="note_id" value="<?= (int)$l['id'] ?>">
                            <input type="hidden" name="new_status" value="active">
                            <button type="submit" class="icon-btn ib-restore" title="Reopen">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.5"/></svg>
                            </button>
                        </form>
                    <?php endif; ?>
...
<div class="panel" id="side-panel">
        ...
        <form method="post" id="followup-form">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                ...
                <div>
                    ...
                    <div id="panel-due-preview" style="font-size:11px;color:var(--orange);margin-top:3px;min-height:14px;"></div>
                </div>
            </div>
        </form>
    </div>
    ...
    <div style="padding:12px 16px;border-top:1px solid var(--border);background:#fff;flex-shrink:0;">
        <div style="display:flex;gap:8px;align-items:flex-end;">
            ...
            <button onclick="sendNote()" style="background:var(--orange);color:#fff;border:none;border-radius:50%;width:40px;height:40px;cursor:pointer;font-size:18px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">↑</button>
        </div>
        <div style="font-size:11px;color:var(--gray);margin-top:5px;padding:0 4px;">Enter to send · Shift+Enter for new line · <a href="#" onclick="saveFollowup();return false;" style="color:var(--orange);">Save follow-up settings</a></div>
    </div>
</div>
...
.bubble-me{background:var(--orange);color:#fff;border-bottom-right-radius:4px;align-self:flex-end;}
...
function openPanel(id) {
    ...
    fetch('', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'action=mark_viewed&lead_id='+d.lead_id});
...
function renderChat(lead_id) {
    ...
    if (notes.length === 0) { noMsg.style.display = 'block'; }
    else {
        ...
        notes.forEach(function(n){
            ...
            row.innerHTML = '<div class="bubble '+(isMe?'bubble-me':'bubble-other')+'">'
                + escHtml(n.note)+'<div class="bubble-meta">'+(isMe?'You':'<strong>'+escHtml(n.username)+'</strong>')+' · '+fmt+'</div></div>';
